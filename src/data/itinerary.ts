...
export interface Day {
  ...
  start?: { time: string; from: string }; // 從前一晚住宿出發的時間與地點（到第一站的車程＝stops[0].driveFromPrev）
  stops: Stop[];
}
...
const MEALS: Record<number, { breakfast?: string; lunch?: string; dinner?: string }> = {
  ...
  2: { breakfast: 'Sandholt 烘焙坊', lunch: 'Friðheimar 蕃茄農場（winebar 免訂／buffet 需訂）；備案 Gullfoss 羊肉湯', dinner: '自理輕食（小木屋）' },
  ...
  6: { breakfast: '自理', lunch: 'Vogafjós 農場餐廳（建議訂位・本日正餐）', dinner: 'Akureyri：Greifinn（披薩/肋排）或 Bautinn（傳統冰島菜）；宵夜 Brynja 冰淇淋' },
...
export const DAYS: Day[] = [
  {
    ...
    stops: [
      ...
      { id: 'd1-rainbow', name: '彩虹街', nameLocal: 'Skólavörðustígur', type: '景點', icon: '🌈', coord: [-21.9325, 64.146], walk: true, timeStart: '14:45', timeEnd: '15:25', desc: '教堂正前方的彩虹大道，兩側是設計小店與咖啡館，最好拍的街景。' },
      { id: 'd1-braudco', name: 'Brauð & Co 肉桂捲', nameLocal: 'Brauð & Co', type: '餐廳', icon: '🥐', coord: [-21.9259781, 64.1440791], walk: true, timeStart: '15:25', timeEnd: '15:45', desc: '冰島最紅的肉桂捲（kanilsnúður）與酸種麵包，就在彩虹街往北一個街區、外牆彩繪很好認。', tips: ['肉桂捲熱門、午後可能售完，看到就買', '也可外帶當隔天早餐'], rating: 4.6 },
      { id: 'd1-laugavegur', name: 'Laugavegur 主街', nameLocal: 'Laugavegur', type: '景點', icon: '🛍️', coord: [-21.9126, 64.143], walk: true, timeStart: '15:45', timeEnd: '16:15', desc: '主購物街先探路（66°North、Icewear、書店），正式採購留到 Day 9。' },
      ...
      { id: 'd1-lodging', name: '夜宿：KEF 機場附近', nameLocal: 'Reykjanesbær Airbnb', type: '住宿', icon: '🏠', coord: [-22.5624, 64.0049], timeStart: '20:30', driveFromPrev: { km: 5, min: 7 }, desc: 'Check-in 後早睡調時差（今晚＝台灣清晨）。' },
    ],
  },
  {
    ...
    hardDeadlines: [],
    flexNotes: ['Brúarfoss 條件式：12:00 前離開 Þingvellir 才順路插入', '天氣差→傍晚瀑布留到 Day 3 早上再玩（住宿就在旁邊）', '午後時間寬鬆，各點可彈性伸縮'],
    start: { time: '07:00', from: 'KEF 機場附近住宿' },
    stops: [
      ...
      { id: 'd2-fridheimar', name: '午餐：Friðheimar 蕃茄農場', nameLocal: 'Friðheimar', type: '餐廳', icon: '🍅', coord: [-20.4449, 64.1775], timeStart: '13:00', timeEnd: '14:30', driveFromPrev: { km: 13, min: 15 }, desc: '地熱溫室裡用餐：蕃茄湯＋現烤麵包吃到飽，坐在蕃茄藤之間。', tips: ['蕃茄湯吃到飽 buffet 需線上訂位（旺季常滿）', '現場 winebar 12:00–22:00 免訂位，可點蕃茄料理吃到同款氛圍', '備案：Gullfoss 遊客中心羊肉湯（免訂）'], rating: 4.6, price: 'buffet 3,740 ISK/人', links: { booking: 'https://www.dineout.is/fridheimar', official: 'https://fridheimar.is' } },
  ...
  },
  {
    ...
    title: '冰川健行＋南岸瀑布',
    summary: '冰川健行 → Mia’s 餐車午餐 → Skógafoss 彩虹瀑布 → DC-3 → Dyrhólaey → 黑沙灘 → Vík 晚餐 → 宿 Skógar 西側',
    driveKm: 160, driveTime: '2h40m', sunrise: '03:38', sunset: '23:05',
    hardDeadlines: ['冰川健行 09:10 報到（09:30 出團）', 'Krónan Vík 21:00 關'],
    flexNotes: ['替代案：黑沙灘 ATV 2h 團（含 DC-3）取代冰川健行，二選一', 'DC-3 是最佳調節閥：落後就整段跳過＝省 1 小時', '住宿在 Skógar 西 7km（861 郵區），從 Vík 回程僅 30 分'],
    start: { time: '08:20', from: 'Seljalandsfoss Horizons 小木屋' },
    stops: [
      { id: 'd3-glacier', name: '冰川健行：索爾黑馬冰川', nameLocal: 'Sólheimajökull', type: '活動', icon: '🧗', coord: [-19.3683, 63.5298], timeStart: '09:30', timeEnd: '12:30', driveFromPrev: { km: 32, min: 35 }, desc: '3 小時冰川健行團（冰上 1–1.5h），冰爪＋冰斧裝備全含。火山灰紋理的冰川地形。', tips: ['⚠️ 必預約（Troll Expeditions / Arctic Adventures）', '09:10 報到（提前 20 分），找掛業者旗的巴士', '8 歲以上可參加；登山鞋可租', '☀️ 7 月為冰川健行最佳季（冰較軟、抓地佳）；冰上仍 5–15°C 有風，穿防風保暖層'], price: '~15,000 ISK', badges: ['需預約'], links: { booking: 'https://troll.is/tour/solheimajokull-3-hour-glacier-hike/' } },
      { id: 'd3-mias', name: '午餐：Mia’s Country Van', nameLocal: 'Sveitagrill Míu', type: '餐廳', icon: '🍟', coord: [-19.505, 63.5244], timeStart: '12:45', timeEnd: '13:30', driveFromPrev: { km: 12, min: 12 }, desc: 'Skógar 旁的紅色炸魚薯條餐車，現點現炸、高評價但常排隊。', tips: ['12:00–16:00 營業', '排太長的備案：Vík 鎮的 Smiðjan Brugghús（漢堡／精釀啤酒，4.4★）'], rating: 4.6, hours: '12:00–16:00' },
      { id: 'd3-skogafoss', name: '斯科加瀑布（彩虹瀑布）', nameLocal: 'Skógafoss', type: '景點', icon: '🌈', coord: [-19.5114, 63.5321], timeStart: '13:40', timeEnd: '14:40', driveFromPrev: { km: 3, min: 6 }, desc: '60m 寬幕瀑布，晴天必出彩虹。右側 527 階登頂觀景台視角完全不同，值得爬。', tips: ['停車 1,000 ISK/8h（2025 起收費）', '隱藏版 Kvernufoss 在旁邊步行 20 分，人少'] },
      { id: 'd3-dc3', name: 'DC-3 飛機殘骸', nameLocal: 'Sólheimasandur Plane Wreck', type: '景點', icon: '🛩️', coord: [-19.3648, 63.4591], routeCoord: [-19.3632, 63.4912], timeStart: '14:55', timeEnd: '15:55', driveFromPrev: { km: 10, min: 12 }, desc: '1973 年迫降的美軍 DC-3 躺在黑沙荒原上。搭接駁車來回（含停留約 1h）。', tips: ['接駁 ~2,500 ISK，每 25 分一班（10:00–17:00）', '走路來回 8km/2h 不建議'], badges: ['備選'], cutPriority: 1 },
      { id: 'd3-dyrholaey', name: '迪霍拉里海蝕拱', nameLocal: 'Dyrhólaey', type: '景點', icon: '🕳️', coord: [-19.113, 63.403], timeStart: '16:20', timeEnd: '17:10', driveFromPrev: { km: 23, min: 25 }, desc: '巨大海蝕拱門＋燈塔＋黑沙灘全景，7 月可近距離看 puffin。', tips: ['7 月已過封鳥期、全日開放', '免費停車（上下兩區）'] },
      { id: 'd3-reynisfjara', name: '雷尼斯黑沙灘', nameLocal: 'Reynisfjara', type: '景點', icon: '🖤', coord: [-19.0716, 63.4057], timeStart: '17:30', timeEnd: '18:20', driveFromPrev: { km: 17, min: 20 }, desc: '玄武岩柱洞穴＋海中石柱群 Reynisdrangar。崖壁上滿是海鸚巢。', tips: ['⚠️ 致命離岸流：看現場黃/橘/紅燈號、永遠不背對海', '停車 1,000 ISK/3h', '出發前查 safetravel.is 浪況'], badges: ['安全'] },
      { id: 'd3-kronan', name: '採買：Krónan Vík', nameLocal: 'Krónan Vík', type: '採買', icon: '🛒', coord: [-18.999, 63.4175], timeStart: '18:30', timeEnd: '18:55', driveFromPrev: { km: 12, min: 12 }, desc: '買齊 D4＋D5 早餐與 D4 午餐三明治材料（Höfn 超市 19:00 關、明天到不了）。', hours: '每日 09:00–21:00' },
      { id: 'd3-sudurvik', name: '晚餐：Suður-Vík', nameLocal: 'Suður-Vík', type: '餐廳', icon: '🍽️', coord: [-19.0082, 63.4223], timeStart: '19:00', timeEnd: '20:15', driveFromPrev: { km: 1, min: 3 }, desc: 'Vík 山坡上的老屋餐廳，招牌羊排與亞洲風味料理。', tips: ['旺季建議訂位', '備案：Smiðjan Brugghús（漢堡／精釀）、Black Crust 活性碳披薩'], rating: 4.5 },
      { id: 'd3-lodging', name: '夜宿：南岸小屋（861 Hvolsvöllur 郵區）', nameLocal: 'Airbnb near Skógar', type: '住宿', icon: '🏠', coord: [-19.6179, 63.5451], timeStart: '20:50', driveFromPrev: { km: 38, min: 30 }, desc: '實際位置在 Skógafoss 西側約 7km 的南岸公路旁（非 Hvolsvöllur 鎮上），回程短。' },
    ],
  },
  {
    day: 4, date: '2026-07-15', weekday: '週三', color: '#847e9e',
    title: '羽毛峽谷・冰河湖＋遊船',
    summary: '羽毛峽谷 → Skaftafell 午餐 → Fjallsárlón → Jökulsárlón 遊船＋鑽石沙灘 → 宿 Höfn',
    driveKm: 350, driveTime: '4h15m', sunrise: '03:35', sunset: '23:00',
    hardDeadlines: ['冰河湖遊船 15:20 出發（提前 20 分換票）', 'Pakkhús 21:00 最後點餐'],
    flexNotes: ['遊船前先逛湖畔＋吃點東西；提前約 20 分到湖畔咖啡館換票', 'Eldhraun、Fjallsárlón 是彈性小點，落後可略', 'Skaftafell 與冰河湖停車用 Parka 同卡付（冰河湖半價）'],
    start: { time: '08:00', from: '南岸小屋（Skógar 西側）' },
    stops: [
      { id: 'd4-eldhraun', name: '埃爾德熔岩原', nameLocal: 'Eldhraun', type: '景點', icon: '🍃', coord: [-18.25, 63.42], timeStart: '09:10', timeEnd: '09:25', driveFromPrev: { km: 78, min: 65 }, desc: '1783 年 Laki 大噴發形成、覆滿厚苔蘚的熔岩原，路邊觀景點快拍。', tips: ['勿踩苔蘚（數十年才長回）'], cutPriority: 3 },
      { id: 'd4-fjadrargljufur', name: '羽毛峽谷', nameLocal: 'Fjaðrárgljúfur', type: '景點', icon: '🪶', coord: [-18.1718, 63.7713], timeStart: '09:55', timeEnd: '11:00', driveFromPrev: { km: 29, min: 28 }, desc: '2km 深綠蛇行峽谷，沿崖緣步道走到頂端觀景台來回約 2km。', tips: ['末段 3km 碎石路（小車可行）', '停車 1,000 ISK'] },
      { id: 'd4-skaftafell-lunch', name: '午餐：Skaftafell 遊客中心', nameLocal: 'Skaftafell Visitor Centre', type: '餐廳', icon: '🥪', coord: [-16.9667, 64.0166], timeStart: '12:05', timeEnd: '12:50', driveFromPrev: { km: 70, min: 65 }, desc: 'Vatnajökull 國家公園 Skaftafell 遊客中心的咖啡與簡餐，是長途中段補給、上廁所的好停點。', tips: ['前晚 Vík 採買的三明治也可在此野餐', '有廁所＋遊客中心；停車 1,000 ISK（Parka 同卡冰河湖半價）'], badges: ['自理'] },
      { id: 'd4-fjallsarlon', name: '小冰河湖', nameLocal: 'Fjallsárlón', type: '景點', icon: '🧊', coord: [-16.385, 64.0186], timeStart: '13:30', timeEnd: '14:10', driveFromPrev: { km: 47, min: 42 }, desc: '人少的迷你冰河湖，冰山離岸更近、冰川壁就在眼前。', cutPriority: 2 },
      { id: 'd4-jokulsarlon', name: '傑古沙龍冰河湖＋遊船', nameLocal: 'Jökulsárlón', type: '活動', icon: '🚤', coord: [-16.2306, 64.0784], timeStart: '14:20', timeEnd: '16:00', driveFromPrev: { km: 10, min: 10 }, desc: '巨型冰山漂浮的冰川潟湖，搭水陸兩棲船滑入冰山群、近看冰川壁與海豹，是此行高潮之一。', tips: ['🚤 遊船 15:20–16:00（水陸兩棲船，40 分）；提前約 20 分到湖畔咖啡館換票', '先在岸邊步道逛冰山、咖啡館吃點東西再上船', '🐦 7 月燕鷗/賊鷗護巢期：岸邊勿靠近草叢鳥巢', '停車 1,000 ISK（Parka 同卡有 Skaftafell 折抵）'] },
      { id: 'd4-diamond', name: '鑽石冰沙灘', nameLocal: 'Diamond Beach', type: '景點', icon: '💎', coord: [-16.1777, 64.0443], timeStart: '16:10', timeEnd: '16:50', driveFromPrev: { km: 2, min: 5 }, desc: '冰山碎塊被海浪推回黑沙灘，像散落的鑽石——出海口對面即達。', tips: ['🐦 東側為 7 月燕鷗/賊鷗巢區，勿靠近、被俯衝舉手離開', '⚠️ 注意海浪與離岸流，勿太靠近水線'] },
      { id: 'd4-pakkhus', name: '晚餐：Pakkhús 海螯蝦', nameLocal: 'Pakkhús', type: '餐廳', icon: '🦞', coord: [-15.2014, 64.2496], timeStart: '18:00', timeEnd: '19:20', driveFromPrev: { km: 78, min: 70 }, desc: 'Höfn 港邊倉庫改建的海螯蝦（langoustine）名店。', tips: ['不收訂位→直接進候位（樓下酒吧等位）', '21:00 最後點餐', '備援：Hafnarbúðin 龍蝦堡（平價）、Ottó'], rating: 4.5, price: '海螯蝦主餐 ~8,000 ISK' },
      { id: 'd4-lodging', name: '夜宿：Höfn', nameLocal: 'Sveitarfélagið Hornafjörður', type: '住宿', icon: '🏠', coord: [-15.2082, 64.2539], walk: true, timeStart: '19:30', desc: '龍蝦小鎮 Höfn 過夜。' },
    ],
  },
  {
    ...
    flexNotes: ['Puffin 傍晚場是推薦主案；累了→改 Day 6 清晨 06:00 出發備援', '93 號山路髮夾彎多、易起霧，慢開', '東峽灣車程長、已留緩衝；午餐到補給之間有空檔可彈性'],
    start: { time: '08:40', from: 'Höfn 住宿' },
    stops: [
      ...
      { id: 'd5-lodging', name: '夜宿：Eiðar 公寓', nameLocal: 'Eiðar', type: '住宿', icon: '🏠', coord: [-14.3565, 65.3737], timeStart: '18:45', driveFromPrev: { km: 41, min: 40 }, desc: 'Egilsstaðir 北邊 15 分鐘、94 號公路上——正好在往 puffin 觀鳥台的路上。晚餐自理。', badges: ['自理'] },
  ...
  },
  {
    ...
    title: 'Dettifoss・米湖地熱區',
    summary: 'Dettifoss → Hverir → Krafla → Vogafjós 午餐 → 米湖景點 → Goðafoss → 宿 Akureyri',
    driveKm: 370, driveTime: '4h45m', sunrise: '03:05', sunset: '23:31',
    hardDeadlines: [],
    flexNotes: ['🍽 晚餐在 Akureyri：Greifinn（披薩/肋排）、Bautinn（傳統冰島菜）；宵夜 Brynja 冰淇淋（到 23:30）', '🦟 7 月中米湖搖蚊大爆發：湖畔點戶外備細目防蚊頭網；無風暖日最擾人，登高（Hverfjall）或有風時較緩，密閉的 Dimmuborgir 最嚴重', '趕→先砍 Hverfjall/Dimmuborgir、再砍 Selfoss、Krafla 縮短', '隧道費 24h 內上 veggjald.is 繳'],
    start: { time: '08:15', from: 'Eiðar 公寓' },
    stops: [
      ...
      { id: 'd6-godafoss', name: '眾神瀑布', nameLocal: 'Goðafoss', type: '景點', icon: '🌀', coord: [-17.5502, 65.6828], timeStart: '18:20', timeEnd: '19:15', driveFromPrev: { km: 53, min: 40 }, desc: '馬蹄形的「眾神瀑布」——西元 1000 年改宗時異教神像被投入此瀑。永晝傍晚光線最美。', tips: ['兩岸都有步道'] },
      { id: 'd6-lodging', name: '夜宿：Akureyri', nameLocal: 'Akureyri', type: '住宿', icon: '🏠', coord: [-18.0907, 65.6826], timeStart: '20:00', driveFromPrev: { km: 45, min: 35 }, desc: '北部之都。晚餐：Greifinn（披薩/肋排）、Bautinn（傳統冰島菜）；宵夜 Brynja 老牌冰淇淋（到 23:30）。', tips: ['Vaðlaheiðargöng 隧道費 2,216 ISK 上 veggjald.is 繳（24h 內）'] },
    ],
  },
  {
    ...
    flexNotes: ['全程最緊的銜接鏈。決策點：16:30 還沒離開 Helluland → 直接跳過 Kolugljúfur', '若取消賞鯨：全天提前 3h，加 Sauðárkrókur 午餐，晚餐 19:00 從容吃', '緩衝來自兩個備選點（Blönduós／Kolugljúfur）：落後就先砍，別硬塞'],
    start: { time: '08:30', from: 'Akureyri 住宿（到 Hof 碼頭停車）' },
    stops: [
      ...
      { id: 'd7-lodging', name: '夜宿：Staður', nameLocal: 'Staður, Hrútafjörður', type: '住宿', icon: '🏠', coord: [-21.079, 65.146], timeStart: '22:05', driveFromPrev: { km: 24, min: 20 }, desc: '1 號公路旁的鄉間住宿，周邊無商店（早餐已在 Akureyri 買好）。' },
    ],
  },
  {
    ...
    flexNotes: ['Búðakirkja、Ytri Tunga 排在 Day 9 早上順路', 'Skarðsvík／Svörtuloft 預設砍（579 號 4km 爛路）', 'Vatnshellir 導覽 45 分（夏季整點場），最晚 15:50 到場報到', '長車程到斯奈山已留緩衝，午後備選點可彈性取捨'],
    start: { time: '08:30', from: 'Staður 住宿（旁即 N1 加油站）' },
    stops: [
      ...
      { id: 'd8-vatnshellir', name: 'Vatnshellir 熔岩洞導覽', nameLocal: 'Vatnshellir Cave', type: '活動', icon: '🕳️', coord: [-23.8183, 64.7479], timeStart: '16:00', timeEnd: '17:00', driveFromPrev: { km: 14, min: 15 }, desc: '深入 Snæfellsjökull 火山底下 8,000 年的熔岩管，《地心歷險記》的入口就設定在這座火山。45 分鐘導覽＝「斯奈山火山體驗」首選。', tips: ['⏱ 導覽固定 45 分鐘（夏季 10:00–18:00 整點場）——時間偏短屬正常，重點是地心歷險的氛圍', '⚠️ 預約 16:00 場；停車場到洞口有一小段路，最晚 15:50 到場報到領裝備', '含頭盔手電筒，5 歲+；洞內約 8°C，帶件外套'], price: '5,900 ISK', badges: ['需預約'], links: { booking: 'https://www.summitguides.is/vatnshellir-cave-tour' } },
      ...
      { id: 'd8-lodging', name: '夜宿：Arnarstapi', nameLocal: 'Arnarstapi / Hellnar', type: '住宿', icon: '🏠', coord: [-23.626, 64.7686], timeStart: '20:00', driveFromPrev: { km: 4, min: 6 }, desc: 'Check-in 後趁永晝走海岸步道。' },
  ...
  },
  {
    ...
    flexNotes: ['明早 05:15 起床→今晚早收、23:00 前睡', '購物時段 2.5 小時可伸縮（最大緩衝來源）'],
    start: { time: '08:25', from: 'Arnarstapi 住宿' },
    stops: [
      ...
      { id: 'd9-checkin', name: 'Check-in：雷克雅維克市區', nameLocal: 'Reykjavík Airbnb', type: '住宿', icon: '🏠', coord: [-21.93, 64.145], timeStart: '13:05', timeEnd: '13:35', driveFromPrev: { km: 75, min: 60 }, desc: '放行李＋停車（P2 230 ISK/h，09:00–21:00 收費）。' },
