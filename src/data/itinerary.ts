...
const MEALS: Record<number, { breakfast?: string; lunch?: string; dinner?: string }> = {
  ...
  6: { breakfast: '自理', lunch: 'Vogafjós 農場餐廳（建議訂位・本日正餐）', dinner: 'Akureyri：Greifinn／Bautinn（溫泉略過→約 20:30 抵達可從容吃；晚到→Brynja 冰淇淋宵夜）' },
...
export const DAYS: Day[] = [
  ...
  },
  {
    ...
    flexNotes: ['Brúarfoss 是條件式：12:00 前離開 Þingvellir 才插入', '天氣差→傍晚瀑布改 Day 3 早上再玩（就住旁邊）', '時間已留緩衝，午後皆可彈性伸縮'],
  ...
  },
  {
    ...
    flexNotes: ['新順序：先冰川健行 → 餐車午餐 → 彩虹瀑布 → 再往東玩', '替代案：黑沙灘 ATV 2h 團（含 DC-3）取代冰川健行，二選一', 'DC-3 可整段跳過＝內建 1 小時緩衝（先冰川後回頭，DC-3 是最佳調節閥）', '住宿實際在 Skógar 西 7km（861 郵區），從 Vík 回程僅 30 分'],
    start: { time: '08:50', from: 'Seljalandsfoss Horizons 小木屋' },
    stops: [
      { id: 'd3-glacier', name: '冰川健行：索爾黑馬冰川', nameLocal: 'Sólheimajökull', type: '活動', icon: '🧗', coord: [-19.3683, 63.5298], timeStart: '09:40', timeEnd: '13:00', driveFromPrev: { km: 32, min: 35 }, desc: '3 小時冰川健行團（冰上 1–1.5h），冰爪＋冰斧裝備全含。火山灰紋理的冰川地形。', tips: ['⚠️ 必預約（Troll Expeditions / Arctic Adventures）', '09:40 報到（提前 20 分），找掛業者旗的巴士', '8 歲以上可參加；登山鞋可租', '☀️ 7 月為冰川健行最佳季（冰較軟、抓地佳）；冰上仍 5–15°C 有風，穿防風保暖層'], price: '~15,000 ISK', badges: ['需預約'], links: { booking: 'https://troll.is/tour/solheimajokull-3-hour-glacier-hike/' } },
      { id: 'd3-mias', name: '午餐：Mia’s Country Van', nameLocal: 'Sveitagrill Míu', type: '餐廳', icon: '🍟', coord: [-19.505, 63.5244], timeStart: '13:15', timeEnd: '14:00', driveFromPrev: { km: 12, min: 12 }, desc: 'Skógar 旁的紅色炸魚薯條餐車，現點現炸、高評價但常排隊。', tips: ['12:00–16:00 營業', '排太長→改 Vík 再吃'], rating: 4.6, hours: '12:00–16:00' },
      { id: 'd3-skogafoss', name: '斯科加瀑布（彩虹瀑布）', nameLocal: 'Skógafoss', type: '景點', icon: '🌈', coord: [-19.5114, 63.5321], timeStart: '14:10', timeEnd: '15:15', driveFromPrev: { km: 3, min: 6 }, desc: '60m 寬幕瀑布，晴天必出彩虹。右側 527 階登頂觀景台視角完全不同，值得爬。', tips: ['停車 1,000 ISK/8h（2025 起收費）', '隱藏版 Kvernufoss 在旁邊步行 20 分，人少'] },
      { id: 'd3-dc3', name: 'DC-3 飛機殘骸', nameLocal: 'Sólheimasandur Plane Wreck', type: '景點', icon: '🛩️', coord: [-19.3648, 63.4591], routeCoord: [-19.3632, 63.4912], timeStart: '15:30', timeEnd: '16:30', driveFromPrev: { km: 10, min: 12 }, desc: '1973 年迫降的美軍 DC-3 躺在黑沙荒原上。搭接駁車來回（含停留約 1h）。', tips: ['接駁 ~2,500 ISK，每 25 分一班（10:00–17:00）', '走路來回 8km/2h 不建議'], badges: ['備選'], cutPriority: 1 },
      { id: 'd3-dyrholaey', name: '迪霍拉里海蝕拱', nameLocal: 'Dyrhólaey', type: '景點', icon: '🕳️', coord: [-19.113, 63.403], timeStart: '16:55', timeEnd: '17:45', driveFromPrev: { km: 23, min: 25 }, desc: '巨大海蝕拱門＋燈塔＋黑沙灘全景，7 月可近距離看 puffin。', tips: ['7 月已過封鳥期、全日開放', '免費停車（上下兩區）'] },
      { id: 'd3-reynisfjara', name: '雷尼斯黑沙灘', nameLocal: 'Reynisfjara', type: '景點', icon: '🖤', coord: [-19.0716, 63.4057], timeStart: '18:05', timeEnd: '18:55', driveFromPrev: { km: 17, min: 20 }, desc: '玄武岩柱洞穴＋海中石柱群 Reynisdrangar。崖壁上滿是海鸚巢。', tips: ['⚠️ 致命離岸流：看現場黃/橘/紅燈號、永遠不背對海', '停車 1,000 ISK/3h', '出發前查 safetravel.is 浪況'], badges: ['安全'] },
      { id: 'd3-kronan', name: '採買：Krónan Vík', nameLocal: 'Krónan Vík', type: '採買', icon: '🛒', coord: [-18.999, 63.4175], timeStart: '19:05', timeEnd: '19:30', driveFromPrev: { km: 12, min: 12 }, desc: '買齊 D4＋D5 早餐與 D4 午餐三明治材料（Höfn 超市 19:00 關、明天到不了）。', hours: '每日 09:00–21:00' },
      { id: 'd3-sudurvik', name: '晚餐：Suður-Vík', nameLocal: 'Suður-Vík', type: '餐廳', icon: '🍽️', coord: [-19.0082, 63.4223], timeStart: '19:35', timeEnd: '20:50', driveFromPrev: { km: 1, min: 3 }, desc: 'Vík 山坡上的老屋餐廳，招牌羊排與亞洲風味料理。', tips: ['旺季建議訂位', '備案：Black Crust 活性碳披薩（12:00–21:00 免訂）'], rating: 4.5 },
      { id: 'd3-lodging', name: '夜宿：南岸小屋（861 Hvolsvöllur 郵區）', nameLocal: 'Airbnb near Skógar', type: '住宿', icon: '🏠', coord: [-19.6179, 63.5451], timeStart: '21:25', driveFromPrev: { km: 38, min: 30 }, desc: '實際位置在 Skógafoss 西側約 7km 的南岸公路旁（非 Hvolsvöllur 鎮上），回程短。', tips: ['座標已用你提供的 Google Maps 連結精確解析'] },
    ],
  },
  {
    ...
    hardDeadlines: ['冰河湖遊船預約時段（建議下午場）', 'Pakkhús 21:00 最後點餐'],
    flexNotes: ['黑瀑布健行已拿掉，改 Skaftafell 遊客中心咖啡簡餐＋上廁所，本日更從容', '冰河湖遊船加回（cn.guidetoiceland.is 可中文線上預訂）：Zodiac 快艇深入冰川壁（10 歲/130cm 以上）／Amphibian 水陸船較親子；旺季提前 2–4 週訂', 'Skaftafell 與冰河湖停車用 Parka 同卡付（冰河湖半價）'],
    ...
    stops: [
      ...
      { id: 'd4-jokulsarlon', name: '傑古沙龍冰河湖＋遊船', nameLocal: 'Jökulsárlón', type: '活動', icon: '🚤', coord: [-16.2306, 64.0784], timeStart: '14:25', timeEnd: '16:30', driveFromPrev: { km: 10, min: 10 }, desc: '巨型冰山漂浮的冰川潟湖，搭船深入冰山群與冰川壁，是此行高潮之一。', tips: ['🚤 遊船二選一：Zodiac 快艇（水上約 30 分、深入冰川壁，限 10 歲/130cm 以上）／Amphibian 水陸船（30–40 分、無年齡限制、較親子）', '⚠️ 旺季 7 月易售罄，提前 2–4 週於 cn.guidetoiceland.is 中文預訂；Zodiac 須提前 30 分換票', '🐦 7 月燕鷗/賊鷗護巢期：岸邊勿靠近草叢鳥巢', '停車 1,000 ISK（Parka 同卡有 Skaftafell 折抵）'], price: 'Zodiac ~17,000 ISK／Amphibian 較平價', badges: ['需預約'], links: { booking: 'https://cn.guidetoiceland.is/book-holiday-trips/zodiac-tour' } },
      { id: 'd4-diamond', name: '鑽石冰沙灘', nameLocal: 'Diamond Beach', type: '景點', icon: '💎', coord: [-16.1777, 64.0443], timeStart: '16:40', timeEnd: '17:20', driveFromPrev: { km: 2, min: 5 }, desc: '冰山碎塊被海浪推回黑沙灘，像散落的鑽石——出海口對面即達。', tips: ['🐦 東側為 7 月燕鷗/賊鷗巢區，勿靠近、被俯衝舉手離開', '⚠️ 注意海浪與離岸流，勿太靠近水線'] },
      { id: 'd4-pakkhus', name: '晚餐：Pakkhús 海螯蝦', nameLocal: 'Pakkhús', type: '餐廳', icon: '🦞', coord: [-15.2014, 64.2496], timeStart: '18:35', timeEnd: '19:55', driveFromPrev: { km: 78, min: 70 }, desc: 'Höfn 港邊倉庫改建的海螯蝦（langoustine）名店。', tips: ['不收訂位→直接進候位（樓下酒吧等位）', '21:00 最後點餐', '備援：Hafnarbúðin 龍蝦堡（平價）、Ottó'], rating: 4.5, price: '海螯蝦主餐 ~8,000 ISK' },
      { id: 'd4-lodging', name: '夜宿：Höfn', nameLocal: 'Sveitarfélagið Hornafjörður', type: '住宿', icon: '🏠', coord: [-15.2082, 64.2539], walk: true, timeStart: '20:05', desc: '龍蝦小鎮 Höfn 過夜。', tips: ['座標為鎮中心近似值'] },
    ],
  },
  {
  ...
  },
  {
    ...
    flexNotes: ['🛁 米湖溫泉改為「備選」：Earth Lagoon 2026 改建中、行前必確認；不泡或未重開→直接早收，約 19:30 到 Goðafoss、20:30 抵 Akureyri 從容吃晚餐', '🍽 晚餐建議在 Akureyri：Greifinn／Bautinn（晚到→Brynja 冰淇淋宵夜）', '🦟 7 月中米湖搖蚊（midge）大爆發：湖畔點戶外備細目防蚊頭網；無風暖日最擾人，登高處（Hverfjall）或有風時較緩，密閉的 Dimmuborgir 最嚴重', '全程最重的一天：趕→先砍 Hverfjall/Dimmuborgir、再砍 Selfoss、Krafla 縮短'],
    ...
    stops: [
      ...
      { id: 'd6-myvatnbaths', name: '米湖溫泉 Earth Lagoon（備選）', nameLocal: 'Jarðböðin við Mývatn', type: '活動', icon: '🛁', coord: [-16.8476, 65.6309], timeStart: '18:00', timeEnd: '20:00', driveFromPrev: { km: 6, min: 8 }, desc: '米湖版藍湖：乳藍色地熱溫泉俯瞰湖景，比藍湖便宜且人少。', tips: ['⚠️ 改建中、2026 夏重開（行前必確認）；未重開或不泡→直接早收，去 Akureyri 吃晚餐', '~7,900 ISK 起；要泡務必提前訂時段票', '備案：Akureyri 的 Forest Lagoon 晚場'], badges: ['需預約', '備選'], cutPriority: 1, links: { booking: 'https://www.earthlagoon.is' } },
      { id: 'd6-godafoss', name: '眾神瀑布', nameLocal: 'Goðafoss', type: '景點', icon: '🌀', coord: [-17.5502, 65.6828], timeStart: '20:45', timeEnd: '21:30', driveFromPrev: { km: 53, min: 40 }, desc: '馬蹄形的「眾神瀑布」——西元 1000 年改宗時異教神像被投入此瀑。永晝傍晚光線最美。', tips: ['兩岸都有步道', '不泡溫泉→約 19:30 到此、20:30 抵 Akureyri 吃晚餐'] },
      { id: 'd6-lodging', name: '夜宿：Akureyri', nameLocal: 'Akureyri', type: '住宿', icon: '🏠', coord: [-18.0907, 65.6826], timeStart: '22:15', driveFromPrev: { km: 45, min: 35 }, desc: '北部之都。宵夜可吃 Brynja 老牌冰淇淋（到 23:30）。', tips: ['🍽 晚餐：Greifinn（披薩/肋排，約到 22:00）、Bautinn（傳統冰島菜）；不泡溫泉→約 20:30 抵達更從容', 'Vaðlaheiðargöng 隧道費 2,216 ISK 上 veggjald.is 繳（24h 內）', '座標為市中心近似值'] },
    ],
  },
  {
  ...
  },
  {
    ...
    flexNotes: ['Búðakirkja＋Ytri Tunga 移到 Day 9 早上順路，本日壓力大減', 'Skarðsvík／Svörtuloft 預設砍（579 號 4km 爛路）', 'Vatnshellir 導覽僅 45 分（夏季整點場），最晚 15:50 到場報到', '長車程到斯奈山已留緩衝，午後備選點可彈性取捨'],
